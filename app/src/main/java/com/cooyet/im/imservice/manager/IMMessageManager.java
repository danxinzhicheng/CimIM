...
import com.cooyet.im.DB.DBInterface;
...
import com.cooyet.im.imservice.event.RefreshHistoryMsgEvent;
import com.cooyet.im.imservice.service.LoadImageService;
...
import com.cooyet.im.protobuf.helper.EntityChangeEngine;
import com.cooyet.im.protobuf.helper.Java2ProtoBuf;
import com.cooyet.im.protobuf.helper.ProtoBuf2JavaBean;
...
public class IMMessageManager extends IMManager {
    ...
    public void ackReceiveMsg(MessageEntity msg) {
        ...
        imSocketManager.sendRequest(imMsgDataAck, sid, cid,false);
    }
...
//                    sendMessage(msg);
     ...
     * @return
     */
    private List<MessageEntity> doLoadHistoryMsg(int pullTimes, final int peerId, final int peerType, final String sessionKey, int lastMsgId, long lastCreateTime, int count) {
        if (lastMsgId < 1 || TextUtils.isEmpty(sessionKey)) {
            return Collections.emptyList();
        }
        if (count > lastMsgId) {
    ...
    private void reqMsgById(int peerId, int sessionType, List<Integer> msgIds) {
        ...
        imSocketManager.sendRequest(imGetMsgByIdReq, sid, cid,false);
    }
    ...
    public void reqHistoryMsgNet(int peerId, int peerType, int lastMsgId, int cnt) {
        ...
        imSocketManager.sendRequest(req, sid, cid,false);
    }
