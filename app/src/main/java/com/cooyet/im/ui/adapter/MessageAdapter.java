package com.cooyet.im.ui.adapter;
...
import com.cooyet.im.config.MessageConstant;
import com.cooyet.im.imservice.entity.AudioMessage;
import com.cooyet.im.imservice.entity.ImageMessage;
import com.cooyet.im.imservice.entity.MixMessage;
import com.cooyet.im.imservice.entity.TextMessage;
import com.cooyet.im.imservice.service.IMService;
import com.cooyet.im.ui.activity.PreviewGifActivity;
import com.cooyet.im.ui.activity.PreviewMessageImagesActivity;
import com.cooyet.im.ui.activity.PreviewMessageLocationActivity;
import com.cooyet.im.ui.activity.PreviewTextActivity;
import com.cooyet.im.ui.helper.AudioPlayerHandler;
import com.cooyet.im.ui.helper.Emoparser;
import com.cooyet.im.ui.helper.listener.OnDoubleClickListener;
import com.cooyet.im.ui.widget.GifView;
import com.cooyet.im.ui.widget.SpeekerToast;
import com.cooyet.im.ui.widget.message.AudioRenderView;
import com.cooyet.im.ui.widget.message.EmojiRenderView;
import com.cooyet.im.ui.widget.message.GifImageRenderView;
import com.cooyet.im.ui.widget.message.ImageRenderView;
import com.cooyet.im.ui.widget.message.MessageOperatePopup;
import com.cooyet.im.ui.widget.message.RenderType;
import com.cooyet.im.ui.widget.message.TextRenderView;
import com.cooyet.im.ui.widget.message.TimeRenderView;
import com.cooyet.im.utils.CommonUtil;
import com.cooyet.im.utils.DateUtil;
import com.cooyet.im.utils.FileUtil;
import com.cooyet.im.utils.ImageUtil;
...
//                    imageMessage.setLoadStatus(MessageStatus.IMAGE_UNLOAD);//如果是图片已经上传成功呢？
            ...
            public void onMsgSuccess() {
                ...
                if (null != imageList) {
                    ...
                    if (curImagePosition != -1) {
                        new Thread(new Runnable() {
                            @Override
                            public void run() {
                                Log.i("kkkk","curImagePosition:"+curImagePosition);
                                String url = imageList.get(curImagePosition).getUrl();
                                Log.i("kkkk","url:"+url);
//                                String path = Environment.getExternalStorageDirectory().getPath() + "/TTIM";
//                                String fileName = "temp.jpg";
//                                ImageUtil.saveFileFromServerToSdcard(url, path, fileName);
//                                String path2 = path + File.separator + fileName;
                                ExifInterface exi = null;
                                try {
                                    exi = new ExifInterface(url);
                                    final float latitude = LocationUtil.convertRationalLatLonToFloat(exi.getAttribute(ExifInterface.TAG_GPS_LATITUDE), exi.getAttribute(ExifInterface.TAG_GPS_LATITUDE_REF));
                                    final float longitude = LocationUtil.convertRationalLatLonToFloat(exi.getAttribute(ExifInterface.TAG_GPS_LONGITUDE), exi.getAttribute(ExifInterface.TAG_GPS_LONGITUDE_REF));
                                    if (latitude != 0 || longitude != 0) {
                                        //如果接受的图片是发送位置的地图，则跳转到地图界面
                                        new Handler(Looper.getMainLooper()).post(new Runnable() {
                                            @Override
                                            public void run() {
                                                Intent i = new Intent(ctx, PreviewMessageLocationActivity.class);
                                                i.putExtra(IntentConstant.CUR_MESSAGE_TAG_GPS_LATITUDE, latitude + "");
                                                i.putExtra(IntentConstant.CUR_MESSAGE_TAG_GPS_LONGITUDE, longitude + "");
                                                ctx.startActivity(i);
