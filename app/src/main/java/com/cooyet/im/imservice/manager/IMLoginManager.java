...
public class IMLoginManager extends IMManager {
    ...
    private void reqLoginOut() {
        ...
        try {
            imSocketManager.sendRequest(imLogoutReq, sid, cid, false);
        } catch (Exception e) {
    ...
    public void reqLoginMsgServer() {
        ...
        /** 加密 */
        String desPwd = new String(com.cooyet.im.Security.getInstance().EncryptPass(loginPwd));
...
//                    IMUdtManager.instance().close(IMUdtManager.handle);
//                    IMUdtManager.instance().cleanup();
